...
### 1.0.2　2026/8/29
- 優化 `components.css` 及 `home.css` ，重新編排內容程式碼，降低位元組。
- 添加 `terms.html` 檔案，製作使用條款，完善本平台使用者保障。

### 1.0.1　2026/8/27
資料來源 (Data Sources)
- **擴充資料來源**：新增更多港口與車站資料來源，提升圖鑑完整度。
- **屏東縣車站納入**：整合屏東縣境內車站，補齊南迴線與屏東線資料。

介面與體驗 (UX Improvements)
- **手機版首頁按鈕布局調整**：重新設計行動端首頁按鈕排列，改善窄螢幕下的操作流暢度。
